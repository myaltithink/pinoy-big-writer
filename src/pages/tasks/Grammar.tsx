...
function Grammar() {
  ...
  useEffect(() => {
    ...
    setLevels(user?.progress.grammar ?? null);
  }, [user]);
  ...
  const renderLevelIcon = (completed: boolean, index: number) => {
    ...
    return completed ? (
      <FaStar key={index} className={`${iconSize} text-blue-400`} />
    ) : (
      <FaRegStar key={index} className={`${iconSize} text-blue-400`} />
    );
  };

  return (
    <div
      className={`w-dvw h-dvh flex flex-col items-center grammar-bg ${
    ...
    >
      ...
      <div className="w-full flex items-center justify-start">
        ...
        >
          ...
          <span
            ...
            } w-fit h-fit text-white bg-blue-400 rounded-full p-2 flex items-center justify-center`}
          >
      ...
      >
        {levels?.map((completed, idx) => {
          ...
          const linkPath = `/tasks/grammar/level-${idx + 1}`;
          const levelContent = (
            <motion.div
              key={idx}
