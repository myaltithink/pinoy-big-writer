...
function Vocabulary() {
  ...
  useEffect(() => {
    ...
    setLevels(user?.progress.vocabulary ?? null);
  }, [user]);
  ...
  const renderLevelIcon = (completed: boolean, index: number) => {
    ...
    return completed ? (
      <FaStar key={index} className={`${iconSize} text-yellow-700`} />
    ) : (
      <FaRegStar key={index} className={`${iconSize} text-yellow-700`} />
    );
  };

  return (
    <div
      className={`w-dvw h-dvh flex flex-col items-center vocabulary-bg ${
    ...
    >
      ...
      <div className="w-full flex items-center justify-start">
        ...
        >
          ...
          <span
            ...
            } w-fit h-fit text-white bg-yellow-600 rounded-full p-2 flex items-center justify-center`}
          >
      ...
      >
        {levels?.map((completed, idx) => {
          ...
          const linkPath = `/tasks/vocabulary/level-${idx + 1}`;
          const levelContent = (
            <motion.div
              key={idx}
